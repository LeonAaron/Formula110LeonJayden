"""Try ReactiveParams overrides against the held-out seed suite, without editing reactive.py.
...
immediately, and only hand-edit reactive.py once a value is confirmed safe.

Usage:
    uv run python scripts/try_params.py --max-speed-mps 18.0
    uv run python scripts/try_params.py --max-speed-mps 18.0 --apex-bias-max-m 0.6
    uv run python scripts/try_params.py  # no overrides: re-checks DEFAULT_PARAMS as-is
"""
